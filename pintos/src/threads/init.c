...
main (void)
{
...
#ifdef USERPROG
  exception_init ();
  syscall_init ();//syscall_handler 수정해줘야함 by 10.29 형준
#endif
  ...
  run_actions (argv);//여기서 프로그램 실행 및 완료
...
parse_options (char **argv) 
{
  for (; *argv != NULL && **argv == '-'; argv++)
    {
      char *save_ptr;
      char *name = strtok_r (*argv, "=", &save_ptr);//나누는 기준이 =
...
run_task (char **argv)
{
  const char *task = argv[1];//[0]:run [1]:'echo x' 10.31형준
